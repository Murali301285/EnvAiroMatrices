...
def migrate():
    ...
    try:
        with conn.cursor() as cursor:
            # 1. Move folder_name from Formatter to JsonMapping
            try:
                cursor.execute("ALTER TABLE tblJsonFormatter DROP COLUMN IF EXISTS folder_name;")
            except: pass
            
            cursor.execute("ALTER TABLE tblDeviceJsonMapping ADD COLUMN IF NOT EXISTS folder_name VARCHAR(255) DEFAULT '';")
            print("Successfully migrated folder_name column to tblDeviceJsonMapping.")

            # 2. Update SP to internalize tvoc tag and use CURRENT_TIMESTAMP explicitly
            sp_sql = """
            DROP FUNCTION IF EXISTS sp_get_woloo_schJsonCreator(character varying, character varying);
            ...
            BEGIN
                -- Grab literal absolute latest record for point-in-time value maps (like latest tvoc, latest IN)
                ...
                -- Windows strictly bound to absolute current system clock as requested!
                ...
                latest_metrics AS (
                    -- Literal point-in-time metrics mapping
                    SELECT 
                ...
                hourly_aggregations AS (
                    -- Aggregations relative to ACTUAL SYSTEM TIMESTAMP WINDOW instead of payload delay!
                    SELECT 
                    ...
                    WHERE tblminutedetails.deviceid = p_deviceid 
                      ...
                      AND createdat <= v_current_time
                ),
                daily_aggregations AS (
                    -- Absolute Current System Day aggregations
                    SELECT 
                        COALESCE(SUM((metrics->>'IN')::NUMERIC), 0) AS pcd,
                        COALESCE(MAX((metrics->>'IN')::NUMERIC), 0) AS pcd_max
                    FROM tblminutedetails
                    WHERE tblminutedetails.deviceid = p_deviceid 
                      AND date(createdat) = date(v_current_time)
                )
                SELECT 
                    di.client::VARCHAR,
                    di.deviceid::VARCHAR,
            ...
            print("Successfully updated sp_get_woloo_schJsonCreator with internal variable bounds and absolute timestamps.")
