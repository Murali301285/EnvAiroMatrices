...
def migrate_database_v13():
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            print("Applying Migration V13: Standardize PCH Alert to 30 Min Blocks...")
            
            # Step 1: Add isAlertProcessedPch column
            try:
                cursor.execute("""
                    ALTER TABLE tblDatareceiver 
                    ADD COLUMN IF NOT EXISTS isAlertProcessedPch NUMERIC DEFAULT 0;
                """)
            except Exception as e:
                print("Note: isAlertProcessedPch column might already exist.", e)
            
            # Step 2: Update sp_get_woloo_schJsonCreator to use 30-min window
            cursor.execute("""
...
AS $function$
            ...
            BEGIN
                ...
                pch_cycles AS (
                    SELECT 
                        date_trunc('hour', m.created_at) + (DIV(EXTRACT(MINUTE FROM m.created_at)::INT, 30) * 30) * INTERVAL '1 minute' AS bucket_start,
                ...
                pch_evaluations AS (
                    ...
                    WHERE m.deviceid = p_deviceid 
                      ...
                      AND m.created_at >= date_trunc('day', v_latest_timestamp)
                    WINDOW w AS (
                        PARTITION BY date_trunc('hour', m.created_at) + (DIV(EXTRACT(MINUTE FROM m.created_at)::INT, 30) * 30) * INTERVAL '1 minute'
