            ...
            BEGIN
                ...
                hourly_pcd AS (
                    SELECT 
                        COALESCE((SELECT (metrics->>'OUT_RAW')::NUMERIC FROM tblminutedetails WHERE tblminutedetails.deviceid = p_deviceid AND created_at >= v_window_start AND created_at <= v_current_time ORDER BY created_at ASC LIMIT 1), 0) AS hour_start_out,
                        COALESCE((SELECT (metrics->>'OUT_RAW')::NUMERIC FROM tblminutedetails WHERE tblminutedetails.deviceid = p_deviceid AND created_at >= v_window_start AND created_at <= v_current_time ORDER BY created_at DESC LIMIT 1), 0) AS hour_current_out,
                        COALESCE((SELECT (metrics->>'OUT')::NUMERIC FROM tblminutedetails WHERE tblminutedetails.deviceid = p_deviceid AND created_at >= v_window_start AND created_at <= v_current_time ORDER BY created_at DESC LIMIT 1), 0) AS device_pch
                ),
                ...
                SELECT 
                    ...
                    json_build_object(
                         'value', CASE WHEN hp.device_pch != GREATEST(0, hp.hour_current_out - hp.hour_start_out) THEN GREATEST(0, hp.hour_current_out - hp.hour_start_out) ELSE hp.device_pch END,
                         'unit', 'count',
                         'condition', 'good',
                         'pch_in', hp.device_pch
                    )::JSON AS pch,
                    CASE WHEN hp.device_pch != GREATEST(0, hp.hour_current_out - hp.hour_start_out) THEN GREATEST(0, hp.hour_current_out - hp.hour_start_out) ELSE hp.device_pch END::NUMERIC AS pch_Avg,
                    CASE WHEN hp.device_pch != GREATEST(0, hp.hour_current_out - hp.hour_start_out) THEN GREATEST(0, hp.hour_current_out - hp.hour_start_out) ELSE hp.device_pch END::NUMERIC AS pch_max,
                    0 AS pch_bad,
