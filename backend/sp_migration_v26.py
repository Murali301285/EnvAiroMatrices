...
DECLARE
    ...
    -- Apply 2-minute execution buffer (so script running at :16 evaluates logically as :14)
    v_current_time     TIMESTAMP := date_trunc('minute', COALESCE(p_ref_time, CURRENT_TIMESTAMP)) - INTERVAL '2 minutes';
...
BEGIN
    ...
    -- Calculate default max lookback. Subtract 1 to make it perfectly inclusive (e.g. 15:15 to 16:14 = 60 mins)
    v_eval_start_time := v_current_time - ((v_timeframe_mins - 1) || ' minutes')::interval;
    ...
    WHERE tmd.deviceid = p_deviceid
      AND (tmd.minute_date + tmd.minute_time) >= v_eval_start_time
      AND (tmd.minute_date + tmd.minute_time) <= v_current_time
