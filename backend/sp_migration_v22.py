...
BEGIN
    ...
    -- The custom python engine (alerts_pch_custom.py) strictly manages the rolling windows,
    -- mid-night rules, and cooldown limits, saving the final valid Net PCH count to tbl_pch_alert.
    -- We read directly from it to guarantee 100% parity with the PCH Logs UI.
    SELECT 
        pa.PchCount,
        pa.created_on,
        pa.isAlertrequired
    INTO v_pch_active_count, v_pch_breached_on, v_is_pch_alert_bool
    ...
    IF v_is_pch_alert_bool = True THEN
        v_is_pch_alert := 'true';
