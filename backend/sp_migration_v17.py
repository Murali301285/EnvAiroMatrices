...
BEGIN
    ...
    -- PCD DIFFERENCE FROM HISTORY (Footfall for current 15 min bucket)
    ...
    IF v_timeframe_mins > 15 THEN
        ...
        FROM (
            ...
            WHERE deviceid = p_deviceid AND payload_type = 'Scheduled'
            ORDER BY slno DESC
            LIMIT v_past_limit
        ) sub;
...
def update_db():
    ...
    cursor.execute("""
        SELECT 1 FROM pg_proc WHERE proname = 'sp_get_woloo_schjsoncreator'
    """)
    ...
    if exists:
        ...
        backup_name = f"backup_{now_str}_sp_get_woloo_schjsoncreator"
        try:
            cursor.execute(f"ALTER FUNCTION public.sp_get_woloo_schjsoncreator(character varying, timestamp without time zone) RENAME TO {backup_name};")
            print(f"Backed up existing SP to: {backup_name}")
        except Exception as e:
            print(f"Backup failed (possibly already renamed today): {e}")
            conn.rollback()
