...
BEGIN
    ...
    WHERE tmd.deviceid = p_deviceid
      AND tmd.created_at >= (v_current_time - (v_timeframe_mins || ' minutes')::interval)
      AND tmd.created_at <= v_current_time
      AND (tmd.metrics->>'OUT_RAW') IS NOT NULL;
      
    -- INITIALIZE JSON WITH ACTIVE COUNT
    v_pch_val_json := json_build_object(
        'pch_count', v_pch_active_count::VARCHAR,
        'pch_prev_alert_on', '',
        'pch_threshold_breached_on', ''
    );
    ...
    IF v_pch_count IS NOT NULL THEN
        ...
        -- Get the previous alert before this one
        SELECT pa2.created_on INTO v_pch_prev_alert
        FROM public.tbl_pch_alert pa2
        WHERE pa2.deviceid = p_deviceid 
          AND pa2.isAlertrequired = True
          AND pa2.created_on < v_pch_breached_on
        ORDER BY pa2.slno DESC
        LIMIT 1;
        
        v_pch_val_json := json_build_object(
            'pch_count', v_pch_active_count::VARCHAR,
            'pch_prev_alert_on', COALESCE(to_char(v_pch_prev_alert, 'YYYY-MM-DD HH24:MI:SS'), ''),
            'pch_threshold_breached_on', to_char(v_pch_breached_on, 'YYYY-MM-DD HH24:MI:SS')
        );
    END IF;
    ...
    hour_accumulating AS (
        SELECT
            ROUND(COALESCE(MAX((tmd.metrics->>'OUT')::NUMERIC) - MIN((tmd.metrics->>'OUT')::NUMERIC), 0), 0) AS pch_max_val,
            ROUND(COALESCE((MAX((tmd.metrics->>'OUT')::NUMERIC) - MIN((tmd.metrics->>'OUT')::NUMERIC)) / NULLIF(COUNT(NULLIF(tmd.metrics->>'OUT', '')), 0), 0), 0) AS pch_avg_val
        FROM tblminutedetails tmd
        WHERE tmd.deviceid = p_deviceid
    ...
    SELECT
        ...
        v_pch_val_json::JSON                                                              AS pch_value,
...
def update_db():
    ...
    # Drop old SP first because return type changed
    ...
    # 2. Update JSON Template in tbljsonformatter
    cursor.execute("SELECT jsontemplate FROM tbljsonformatter WHERE name='woloo_scheduled_json'")
    res = cursor.fetchone()
    if res:
        template = json.loads(res[0])
        
        # Insert the two fields next to alert_sequence
        new_template = {}
        for k, v in template.items():
            new_template[k] = v
            if k == "alert_sequence":
                new_template["is_pch_alert"] = "$is_pch_alert"
                new_template["pch_value"] = "$pch_value"
                
        cursor.execute(
            "UPDATE tbljsonformatter SET jsontemplate = %s WHERE name = 'woloo_scheduled_json'",
            (json.dumps(new_template, indent=2),)
        )
        print("Updated JSON Template.")
