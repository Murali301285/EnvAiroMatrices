...
export default function Users() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [usersRes, custRes] = await Promise.all([
                axios.get('http://97.74.92.23:8381/admin/users'),
    ...
    const handleSave = () => {
        ...
        const req = editingId
            ? axios.put(`http://97.74.92.23:8381/admin/users/${editingId}`, payload)
            : axios.post('http://97.74.92.23:8381/admin/users', payload);
    ...
    const handleDelete = (slno: number) => {
        ...
        setLoading(true);
        axios.delete(`http://97.74.92.23:8381/admin/users/${slno}`)
            .then(res => {
                if (res.data.status === 'success') {
