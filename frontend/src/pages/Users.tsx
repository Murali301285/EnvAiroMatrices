...
export default function Users() {
    ...
    const handleSave = () => {
        ...
        const req = editingId
            ? axios.put(`http://localhost:8000/admin/users/${editingId}`, payload)
            : axios.post('http://localhost:8000/admin/users', payload);
    ...
    const handleDelete = (slno: number) => {
        ...
        setLoading(true);
        axios.delete(`http://localhost:8000/admin/users/${slno}`)
            .then(res => {
                if (res.data.status === 'success') {
