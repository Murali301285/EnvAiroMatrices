...
export default function Scheduler() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [devRes, schRes] = await Promise.all([
                ...
                axios.get('http://localhost:8000/admin/schedulers')
            ]);
    ...
    const handleSave = () => {
        ...
        const req = editingId
            ? axios.put(`http://localhost:8000/admin/schedulers/${editingId}`, payload)
            : axios.post('http://localhost:8000/admin/schedulers', payload);
    ...
    const handleDelete = (slno: number) => {
        ...
        setLoading(true);
        axios.delete(`http://localhost:8000/admin/schedulers/${slno}`)
            .then(res => {
                if (res.data.status === 'success') {
